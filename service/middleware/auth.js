...
router.post('/registration', async (req, res) => {
    try {
        if (req.query.phone && req.query.name && req.query.surname && req.query.login && req.query.password && req.query.repeatPassword) {

            const { phone, name, surname, login, password, repeatPassword } = req.query
            ...
            if (password !== repeatPassword) {
                ...
                if (!validateLogin(login)) {
                    ...
                    res.send({ status: 400, message: 'Логин не соответствует к требованиям' })

                } else if (!validatePassword(password, login)) {

                    res.send({ status: 400, message: 'Пароль не соответствует к требованиям' })

                } else {
                    const user = await USERS.findOne({ login })

                    if (user) {
                        res.send({ status: 400, message: 'Пользователь с таким логином уже существует' })
                    } else {
                        ...
                        const token = jwt.sign({ userId: candidate.userId }, 'auth', { expiresIn: '6h' })
            ...
            try {
                ...
                if (user) {
                    ...
                    if (await bcrypt.compare(password, user.password)) {
                        ...
                        const token = jwt.sign({ userId: user.userId }, 'auth', { expiresIn: '6h' })
...
function validatePassword(password, login) {
    ...
    return countLetter >= 2 && countDigit >= 1 && countUpperLetter >= 1 && countLowerLetter >= 1
}
